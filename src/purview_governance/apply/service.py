"""Safe explicit apply workflow (dry-run default) for plan/v1 and plan/v2."""
...
from purview_governance.remote_state.service import capture_remote_state, capture_remote_state_v2
...
def execute_governance_plan(
    ...
    *,
    ...
    - plan/v1 → ``ExecutionResult`` (purview-execution-result/v1)
    - plan/v2 → ``ExecutionResultV2`` (purview-execution-result/v2)
    """
    ...
    if plan.api_version == PLAN_API_VERSION:
        return _execute_governance_plan_v1(plan, client, mode=mode)
    if plan.api_version == PLAN_API_VERSION_V2:
        return _execute_governance_plan_v2(plan, client, mode=mode)
    raise ApplyValidationError(
        "apply.unsupported_plan_version",
        "apply supports purview-governance-plan/v1 and purview-governance-plan/v2 only",
    )


def _execute_governance_plan_v1(
    plan: GovernancePlan,
    client: PurviewScanningClient,
    *,
    mode: ExecutionMode,
) -> ExecutionResult:
    _revalidate_plan_v1(plan)
    mode = _coerce_mode(mode)

    planned_target = plan.target_context.identity
    planned_remote = plan.identities.remote_state
    not_run_ops = _not_run_operations_v1(plan)

    if plan.execution_eligibility == "blocked":
        return _finalize_result_v1(
...
def _execute_governance_plan_v2(
    plan: GovernancePlan,
    client: PurviewScanningClient,
    *,
    mode: ExecutionMode,
) -> ExecutionResultV2:
    _revalidate_plan_v2(plan)
    mode = _coerce_mode(mode)

    planned_target = plan.target_context.identity
    planned_remote = plan.identities.remote_state
    not_run_ops = _not_run_operations_v2(plan)

    if plan.execution_eligibility == "blocked":
        return _finalize_result_v2(
            plan_identity=plan.plan_identity,
            planned_target=planned_target,
            execution_target=None,
            planned_remote=planned_remote,
    ...
    for operation in plan.operations:
        ...
        if operation.resource_type not in {
            "dataSource",
            "classificationRule",
            "scanRuleSet",
            "scan",
    ...
    intents_failed = False
    intents: tuple[MutationIntentV2, ...] = ()
    try:
    ...
    observed_remote: str | None = None
    try:
        remote_state = capture_remote_state_v2(client)
        observed_remote = remote_state.material_state_identity
    except AuthenticationError:
        capture_failed_code = "apply.authentication_failed"
    except RemoteStateError:
...
def _revalidate_plan_v1(plan: GovernancePlan) -> None:
    if plan.api_version != PLAN_API_VERSION:
        raise ApplyValidationError(
            "apply.unsupported_plan_version",
...
def _revalidate_plan_v2(plan: GovernancePlan) -> None:
    if plan.api_version != PLAN_API_VERSION_V2:
        raise ApplyValidationError(
            "apply.unsupported_plan_version",
            "apply supports purview-governance-plan/v2 only on this path",
        )
    failed = False
    try:
        document = plan.to_document()
        if document.get("apiVersion") != PLAN_API_VERSION_V2:
            raise ApplyValidationError(
                "apply.unsupported_plan_version",
                "apply supports purview-governance-plan/v2 only on this path",
            )
...
def _not_run_operations_v1(plan: GovernancePlan) -> tuple[OperationResult, ...]:
    return tuple(
        OperationResult(
...
def _not_run_operations_v2(plan: GovernancePlan) -> tuple[OperationResultV2, ...]:
    return tuple(
        OperationResultV2(
            sequence=operation.sequence,
            resource_type=operation.resource_type,  # type: ignore[arg-type]
            name=operation.name,
            action=operation.action,
            status="not-run",
            data_source_name=operation.data_source_name,
        )
        for operation in plan.operations
    )


def _apply_intents_v1(
    *,
    plan: GovernancePlan,
    client: PurviewScanningClient,
    intents: tuple[MutationIntent, ...],
    ...
    for index, intent in enumerate(intents):
        attempted += 1
        outcome = _invoke_put_v1(client, intent)
...
def _apply_intents_v2(
    *,
    ...
    op_results: list[OperationResultV2] = [
        OperationResultV2(
            sequence=intent.sequence,
            ...
            action=intent.action,
            status="not-run",
            data_source_name=intent.data_source_name,
        )
        for intent in intents
    ]
    performed = 0
    ...
    for index, intent in enumerate(intents):
        attempted += 1
        outcome = _invoke_put_v2(client, intent)
        if outcome == "succeeded":
            performed += 1
            op_results[index] = OperationResultV2(
                sequence=intent.sequence,
                resource_type=intent.resource_type,
                name=intent.name,
                action=intent.action,
...
def _invoke_put_v1(client: PurviewScanningClient, intent: MutationIntent) -> str:
    try:
...
def _invoke_put_v2(client: PurviewScanningClient, intent: MutationIntentV2) -> str:
    try:
        if intent.resource_type == "dataSource":
            receipt = client._create_or_replace_data_source(intent.name, intent.payload)
            status_code = receipt.status_code
        elif intent.resource_type == "classificationRule":
            receipt = client._create_or_replace_classification_rule(intent.name, intent.payload)
            status_code = receipt.status_code
        elif intent.resource_type == "scanRuleSet":
            receipt = client._create_or_replace_scan_rule_set(intent.name, intent.payload)
            status_code = receipt.status_code
        elif intent.resource_type == "scan":
            assert intent.data_source_name is not None
            receipt = client._create_or_replace_scan(
                intent.data_source_name,
                intent.name,
                intent.payload,
            )
            status_code = receipt.status_code
...
def _finalize_result_v2(
    *,
    ...
    validate_result_document_v2_for_serialization(result.to_document())
    return result
