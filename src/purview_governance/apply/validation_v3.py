"""Schema + semantic integrity for purview-execution-result/v3."""
...
from purview_governance.apply.models_v3 import FAILURE_CODES_BY_STATUS_V3
from purview_governance.apply.schema import load_execution_result_v3_schema
    ...
    if status in {
        ...
        if writes_performed <= 0:
            _integrity(
                ...
                "partial requires writes_performed > 0",
            )
        if writes_attempted != writes_performed:
            _integrity(
                "apply.invalid_result_counters",
                "partial requires attempted=performed",
            )
        if writes_unknown != 0:
            _integrity(
                "apply.invalid_result_counters",
                "partial requires unknown=0",
            )
        if writes_performed > n:
            _integrity(
                "apply.invalid_result_operations",
                "partial performed count is inconsistent with operations",
            )
        expected = ["succeeded"] * writes_performed + ["not-run"] * (n - writes_performed)
...
def validate_result_document_semantics(document: dict[str, Any]) -> None:
    ...
    if document.get("apiVersion") != RESULT_API_VERSION_V3:
        _integrity(
    ...
    if status not in FAILURE_CODES_BY_STATUS_V3:
        _integrity("apply.invalid_result_status", "unknown execution status", path="/status")
    ...
    for index, op in enumerate(operations):
        ...
        if normalize_uuid_string(op.get("id")) is None:
            _integrity(
                "apply.invalid_result_operations",
                "operations require id UUID",
                path=f"/operations/{index}/id",
            )
        if "dataSourceName" in op:
            _integrity(
                "apply.invalid_result_operations",
                "dataSourceName is not allowed in v3 operations",
                path=f"/operations/{index}/dataSourceName",
            )
        if "name" in op:
            _integrity(
                "apply.invalid_result_operations",
                "name is not allowed in v3 operations",
    ...
    allowed = FAILURE_CODES_BY_STATUS_V3[status]
    if allowed is None:
    ...
    if status == "blocked":
        ...
        failure_code = None
        failure_raw = document.get("failure")
        if isinstance(failure_raw, dict):
            failure_code = failure_raw.get("code")
        if failure_code == "apply.tenant_binding_unsupported":
            if execution_target is not None and execution_target != planned_target:
                _integrity(
                    "apply.invalid_result_target",
                    "failed-before-write requires matching execution target",
                    path="/executionTargetContextIdentity",
                )
        elif execution_target is None or execution_target != planned_target:
            _integrity(
                "apply.invalid_result_target",
                "failed-before-write requires matching execution target",
                path="/executionTargetContextIdentity",
        ...
        if mode != "apply":
            _integrity("apply.invalid_result_mode", "partial requires mode apply", path="/mode")
        if execution_target is None or execution_target != planned_target:
            _integrity(
                "apply.invalid_result_target",
                "partial requires matching execution target",
                path="/executionTargetContextIdentity",
            )
        if observed is None or observed != planned_remote:
            _integrity(
                "apply.invalid_result_remote",
                "partial requires observed == planned remote identity",
                path="/observedRemoteStateIdentity",
            )
        if performed <= 0:
            _integrity(
        ...
        if attempted != performed or unknown != 0:
            _integrity(
                "apply.invalid_result_counters",
                "partial requires attempted=performed and unknown=0",
            )
        if failed != 0 or unknowns != 0:
            _integrity(
                "apply.invalid_result_operations",
                "partial requires no failed or unknown operations",
            )
        if failure is None:
            _integrity(
                "apply.invalid_result_failure",
                "partial requires failure",
                path="/failure",
            )
...
def validate_result_document_v3_for_serialization(document: dict[str, Any]) -> None:
