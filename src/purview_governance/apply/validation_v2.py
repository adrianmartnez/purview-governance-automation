"""Schema + semantic integrity for purview-execution-result/v2."""
...
from purview_governance.apply.schema import load_execution_result_v2_schema
...
def validate_result_document_schema(document: dict[str, Any]) -> None:
    schema_failed = False
    try:
        schema = load_execution_result_v2_schema()
...
def validate_result_document_semantics(document: dict[str, Any]) -> None:
    ...
    if document.get("apiVersion") != RESULT_API_VERSION_V2:
        _integrity(
    ...
    for index, op in enumerate(operations):
        ...
        elif "dataSourceName" in op:
            _integrity(
                "apply.invalid_result_operations",
                "dataSourceName is only allowed for scan operations",
                path=f"/operations/{index}/dataSourceName",
            )
...
def validate_result_document_v2_for_serialization(document: dict[str, Any]) -> None:
    validate_result_document_schema(document)
    validate_result_document_semantics(document)


def dumps_result_v2_canonical(document: dict[str, Any]) -> str:
